...
def backfill_external_ids(
    dry_run: bool = typer.Option(True, "--dry-run/--execute", help="Preview changes without making API calls"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Enable verbose logging"),
    hire_report: Optional[str] = typer.Option(None, "--hire-report", help="Path to a hire report with employee data"),
    manual_csv: Optional[str] = typer.Option(None, "--csv", help="Path to CSV with columns: name, hire_date (MM-DD-YYYY format)")
):
    """
    ...
    for driver in drivers_needing_ids:
        ...
        if employee_info:
            ...
            if dry_run:
                log.info(f"[DRY RUN] Would add paycomname:{paycom_key} to {driver_name}")
                stats["updated"] += 1
            else:
    ...
    if stats["no_hire_date"]:
        ...
        typer.echo("   3. Run: python migrate_external_ids.py backfill-external-ids --csv your_file.csv")
...
def verify(
    ...
    typer.echo(f"   With paycomname ID: {len(with_paycom)} ({len(with_paycom)*100/len(all_drivers):.1f}%)")
    typer.echo(f"   Without paycomname ID: {len(without_paycom)} ({len(without_paycom)*100/len(all_drivers):.1f}%)")
...
def add_single(
    ...
    if dry_run:
        typer.echo(f"[DRY RUN] Would add paycomname:{paycom_key} to {driver_found['name']}")
    else:
        success = add_external_id_to_driver(driver_found["id"], "paycomname", paycom_key)
        if success:
            typer.echo(f"✅ Successfully added paycomname:{paycom_key} to {driver_found['name']}")
        else:
